...
<div
    ...
    <div class="dz-flex dz-flex-col dz-items-start dz-h-full dz-w-full dz-justify-center dz-bg-transparent dz-dark:border-gray-600 dz-dark:hover:border-gray-500">
        ...
        <div @click="$refs.input.click()" class="dz-border dz-border-dashed dz-rounded dz-border-gray-500 dz-w-full dz-cursor-pointer">
            <div>
                <div x-show="!isDragging" class="dz-flex dz-items-center dz-bg-gray-50 dz-justify-center dz-gap-3 dz-py-8 dz-h-full dz-dark:bg-gray-700">
                    ...
                    <p class="mb-0 dz-text-sm dz-md:text-base dz-text-gray-600 dz-dark:text-gray-400">Drop here or <span class="dz-font-semibold dz-text-black dz-dark:text-white">Browse files</span></p>
                </div>
        ...
        @if((isset($files) || isset($oldDocs)) && (count($files) > 0 || count($oldDocs) > 0))
        <div class="dz-flex dz-flex-wrap dz-gap-x-10 dz-gap-y-2 dz-justify-start dz-w-full dz-mt-5">
            @foreach($oldDocs as $oldIndex => $file)
            <div class="dz-flex dz-items-center dz-justify-between dz-gap-2 dz-border dz-rounded dz-border-gray-200 dz-w-full dz-h-auto dz-overflow-hidden dz-dark:border-gray-700" id="{{ $file['name'] }}">
                ...
                <div class="dz-flex dz-items-center dz-mr-3">
                    <button type="button" @click="removeOldUpload('{{ $file['id'] }}')">
    ...
    <script>
        Alpine.data('dropzone', ({
            ...
            return ({
                ...
                removeOldUpload(value) {
                    Livewire.dispatch('removeOldUpload', [value]);
                    Livewire.dispatch('removeOldDocEntryInDropzone', [value]);
                    oldDocs.splice(value, 1); // remove 1 item at the given index
                }
            });
        })
    </script>
